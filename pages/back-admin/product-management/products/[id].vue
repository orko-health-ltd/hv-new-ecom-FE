          ...
          <Breadcrumb class="hidden md:flex">
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink as-child>
                  <nuxt-link
                    :to="`/back-admin/product-management/products/${route.params.id}`"
        ...
        <div class="mt-3">
          <Card class="bg-white rounded-lg shadow-md">
            ...
            <div class="grid gap-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                  ...
                  <div class="col-span-2 space-y-2">
                    <Label
                      ...
                      >Specification</Label
                    >
                    <p class="text-gray-700">{{ product.specification }}</p>
                  </div>
                  ...
                  <div class="space-y-2">
                    <Label for="sku" class="text-sm font-medium text-gray-700"
                      >Features</Label
                    >
                  ...
                  <div class="col-span-2 space-y-2">
                    <Label for="sku" class="text-sm font-medium text-gray-700"
                      >Brewing Guides</Label
                    >
                    <ul class="list-decimal pl-5 text-gray-700">
                      <li v-for="item in product.brewing_guide" :key="item">
                        {{ item }}
                      </li>
                    </ul>
                  </div>
                  ...
                  <div class="col-span-2 space-y-2">
                    ...
                    <div
                      class="grid items-center justify-center grid-cols-6 gap-4"
                    >
                      <div
                        v-for="image in (product.product_images as Array<{url: string,_id:string}>)"
              ...
              <CardFooter
                class="border-t flex space-y-3 flex-col border-gray-200 p-6"
              >
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow
                      v-for="stock in product.stock_info"
                      :key="stock._id"
                    >
...
const getSkus = async () => {
  try {
    const { data } = await $fetch<{ data: Sku[] }>('/api/back-admin/sku')
    await nextTick()
    if (data.length) {
      skus.value = data
    }
...
const getBrands = async () => {
  try {
    const { data } = await $fetch<{ data: Brand[] }>('/api/back-admin/brands')
...
const getCategories = async () => {
  try {
    const { data } = await $fetch<{ data: Sku[] }>('/api/back-admin/categories')
...
const updateProduct = async () => {
  try {
    ...
    const { data, error } = await useFetch(`/api/back-admin/products/edit`, {
      method: 'POST',
      body: formData,
    })
...
const uploadImage = async (event: Event) => {
  ...
  if (files && files.length > 0) {
    ...
    try {
      const response = await fetch(`/api/back-admin/products/addImage`, {
        method: 'POST',
        body: formData,
      })
      if (!response.ok) {
